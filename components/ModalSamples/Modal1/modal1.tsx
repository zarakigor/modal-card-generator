...
function Modal1() {
  const { contentOfModal1, setContentOfModal1 } = useContext(Context);
  return (
    <div className={styles.modal}>
      ...
      <h2 className="text-3xl font-bold mt-7 mb-5">
        {contentOfModal1.content1}
      </h2>
      <p className="text-xl mb-10">{contentOfModal1.content2}</p>
      <input
        type="input"
        className="form-input border-gray-300 rounded-lg w-full"
        value={contentOfModal1.content3}
      />
      <div className="flex justify-between w-full mt-8">
        <SecondaryButton
          value={{
            text: `${contentOfModal1.content4}`,
            className: "w-6/12 mr-3",
          }}
        />
        <PrimaryButton
          value={{ text: `${contentOfModal1.content5}`, className: "w-6/12" }}
        />
      </div>
    </div>
  );
